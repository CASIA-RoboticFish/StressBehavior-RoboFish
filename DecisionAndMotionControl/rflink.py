'''
This file is used to define RF communication-related protocols. 
The host computer receives sensory information from the robotic fish and sends motion commands, relying on this file for decoding or encoding.
'''
...
# Command enumeration type (set: 3-28, read: 29-40, goto: 41-43)
...
class RFLink():
    ...
    Communication Protocol Specification: (A complete data frame is as follows:)
    2022.04.06 Custom modifications:
    0xFF, SENDER_ID, RECEIVER_ID, RECEIVE_LEN, RECEIVE_PACKAGE, RECEIVE_CHECK

    2021.10.11 Custom modifications:
    :arg length: Message length
    :arg message: Message (byte type)

    :attributes RFLink_receivedata: Receive state machine, decode RFLink communication protocol
    :attributes RFLink_packdata: Package the data to be sent according to the RFLink communication protocol

    2023 Protocol modification for lateral line data reception:
    ...
    def RFLink_receivedata(self, rx_data):
        ...
        :No cmd, after the data length there are directly 12 sensors with a total of 24 bits of data
        """

        if self._receive_state == Recstate.WAITING_FF:
            if rx_data == b'\xff':
                self._receive_state = Recstate.SENDER_ID
                #self._checksum = ord(rx_data)  # Convert to ASCII code
                self.message = b''
                self.length = 0
                self._byte_count = 0
    ...
    def analysis_data(self,with_cmd = 0):  # 分析串口接收到的rflink数据,更新robosharkstate的状态
        ...
        decodes the data based on the command description in the data frame.
        If with_cmd=1:
        :return: The received Command ID
        Otherwise:
        return 0, directly perform data conversion
        """
        ...
        if with_cmd ==1:
            ...
            # The data uploaded by the lower-level machine is stored in sequence with high and low 8 bits
            ...
            data_pack = self.message
            self.data_int_pack = np.zeros(int(Len))

            for i in range(0, Len):
                a = int(data_pack[i])
                self.data_int_pack[i] = a
    ...
    def RFLink_packdata(self, cmd, databyte):
        ...
        :return: Message packet that complies with the RFLink communication protocol
        """
        first_byte = b'\xff'
